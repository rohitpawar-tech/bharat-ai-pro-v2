...
def web_search(query: str) -> str:
    ...
    try:
        # Using DDGS as per the latest library standard
        with DDGS() as ddgs:
...
def build_prompt(conversation_history: list, user_message: str, search_context: str) -> str:
    ...
    # 2️⃣ Conversation Memory System
    for msg in conversation_history:
        role = msg['role'] # 'user' or 'assistant'
        content = msg['content']
        prompt += f"<|start_header_id|>{role}<|end_header_id|>\n\n{content}<|eot_id|>"
    
    # 3️⃣ Web Search AI Integration
    if search_context:
        ...
        for line in response.iter_lines():
            if line:
                ...
                if "response" in json_data:
                    ...
                    full_text += chunk
                    # Yield chunk immediately for frontend typing effect
                    yield chunk
                
                if json_data.get("done", False):
                    break
        
        # Note: We don't apply 'clean_response' mid-stream, 
        # but the frontend could do post-processing if desired.
        # For now, we rely on the prompt instructions to reduce fillers.
